...
<style>
.star-input>.input1,
.star-input>.input1>input:focus+label,
.star-input>.input1>input:checked+label{
...
.star-input>.input1>input{
    position: absolute;
    width: 1px;
    height: 1px;
    opacity: 0;
}
.star-input>.input1.focus{
    outline: 1px dotted #ddd;
}
.star-input>.input1>label{
    width: 10px;
    height: 0;
    padding: 19px 0 0 0;
    overflow: hidden;
...
.star-input>.input1>label[for="p1"]{ 
    width: 20px;
    z-index: 9;
}
.star-input>.input1>label[for="p2"]{ 
    width: 40px;
    z-index: 7;
}
.star-input>.input1>label[for="p3"]{ 
    width: 60px;
    z-index: 5;
}
.star-input>.input1>label[for="p4"]{ 
    width: 80px;
    z-index: 3;
}
.star-input>.input1>label[for="p5"]{ 
    width: 100px;
    z-index: 1;
}
div#form{
...
<div id="form">
 ...
 try{
	...
	pstmt = con.prepareStatement("select * from reply where code=? order by id desc");
	pstmt.setInt(1,code);
	...
	<tr>
		<td rowspan=2 height=50px width=400px>
			<span class="star-input">
			  <span class="input1">
			  <%for(int a=1; a<=5; a++){ 
			  if(a<=rs.getInt("score")){
			  %>
			    <input type="radio" name="star-input<%=i%>" id="p<%=a %>" value="<%=a %>" checked><label for="p<%=a %>"><%=a%></label>
			  <%} else{%>
			  <input type="radio" name="star-input<%=i%>" id="p<%=a %>" value="<%=a %>"><label for="p<%=a %>"><%=a%></label>
			  <%}
			  }%>
			  </span>
			  <span>
		...
		<form method="post" action="delreply.jsp" >
			...
			<input type="submit" value="X" onclick="delreply()" >
		</form>
		</td>
		<%} %>
	</tr>
...
<%}
	...
	pstmt1=con.prepareStatement("update room set grade=? where id=?");
