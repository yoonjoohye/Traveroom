...
function send(){
  ...
  $(document)
    ...
    .on("change", ".star-input>.input", function(){
...
table#roominfo input[type=button]{
	width:150px;
	height:50px;
	border:0px;
...
table#roominfo input[type=button]:hover{
...
table#review input[type=button]{
	width:100%;
	height:100px;
	border:0px;
	font-size:12pt;
...
table#review input[type=button]:hover{
	background-color:#cccccc;
}
</style>

<form name="replyForm" method="post" action="replyAction.jsp">
<%
try {
    // 드라이버 로딩
	...
	<img src="traveroom_img/<%=rs.getString("imgpath")%>" width=100% height=650px>
	<div id="form">
	<table id="roominfo" border>
	   	<tr>
	   		<td>
	   		<small><%=rs.getString("city") %>의 <%=rs.getString("type") %></small>
	   		</td>
	   	</tr>
	   	<tr style="border-bottom:1px solid #dddddd;">
	   		<td style="border-bottom:1px solid #dddddd;"><b style="font-size:30pt;"><%=rs.getString("name") %></b></td>
	   	</tr>
	   	<tr>
	   		<td height=250px style="border-bottom:1px solid #dddddd;"><p style="font-size:15pt;"><%=rs.getString("ex") %></p></td>
	   	</tr>
	   	<tr>
	   		<td height=50px style="border-bottom:1px solid #dddddd;"><span style="font-size:12pt;">연락처 <%=rs.getString("num") %></span></td>
	   	</tr>
	   	<tr>
	   		<td height=100px style="border-bottom:1px solid #dddddd;">
	   			<span style="font-size:15pt;">최대 인원 <%=rs.getInt("member") %>명</span><br>
	   			<small>인원이 초과되면 한명당 10,000원씩 추가요금이 발생합니다.</small>
	   		</td>
	   	</tr>
	   	<tr>
	   		<td height=50px style="border-bottom:1px solid #dddddd;">
	   			<span style="font-size:15pt;"><%=rs.getString("price") %>원</span>
	   		</td>
	   	</tr>
	   	<tr>
	   		<td align="right"><input type="button" onclick="<%=possible%>" value="예약하기"></td>
	   	</tr>
	</table>
...
<table id="review">
	...
	<tr>
		<td width=80%>
			<textarea cols=5 rows=100 name="reply"></textarea>
		</td>
		<td width=15%>
